...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--set",
        dest="set_version",
        help="Set an explicit version (major.minor.patch) instead of bumping.",
    )
    parser.add_argument(
        "--addon-dir",
        default="addon",
...
def main(argv: list[str]) -> int:
    ...
    if args.set_version:
        try:
            version = validate_version(args.set_version)
            print(f"Setting version: {version}")
            sync_version(version, addon_dir)
            print(f"Successfully updated manifest.json and VERSION to {version}")
            return 0
        except Exception as e:
            print(f"Failed to set version: {e}")
            return 1
    return bump_version(addon_dir, args.part)
